<template>
  <div>
    ...
    <van-cell title="小区名称" is-link> 请输入小区名称 </van-cell>
    <van-cell-group>
      <van-field v-model="value1" label="租金" placeholder="请输入租金/月">
        <template #right-icon> ￥/月 </template>
      </van-field>
      <van-field v-model="value1" label="建筑面积" placeholder="请输入建筑面积">
    ...
    <van-cell title="房屋图像" />
    <van-cell class="cell-house">
      <template #title>
        <van-uploader v-model="fileList" multiple class="cell-img" />
      </template>
    </van-cell>
    <van-cell title="房屋配置" />
    <van-grid :column-num="5" class="grid-cell">
      <van-grid-item
        v-for="value in 10"
        :key="value"
        icon="photo-o"
